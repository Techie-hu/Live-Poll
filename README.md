...
- **Web App (Frontend)** — 🚧 _Not yet deployed._ The Next.js app builds cleanly (`next build` ✅) and is ready to host. Click below to one-click-deploy, then paste the resulting URL into this line. The repo ships with a `vercel.json` (`rootDirectory: web`) and a `netlify.toml` (`base = web`) so the subdirectory is detected automatically — no manual config required:
  ...
  - [![Deploy to Netlify](https://www.netlify.com/img/deploy/button.svg)](https://app.netlify.com/start/deploy?repository=https%3A%2F%2Fgithub.com%2FTechie-hu%2FLive-Poll)
